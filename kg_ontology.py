...
class BattleBuddyKG:
    ...
    def _rebuild_from_db(self):
        ...
        for row in self.conn.execute("SELECT * FROM edges"):
            props = json.loads(row["properties"] or "{}")
            self.G.add_edge(row["source"], row["target"], key=row["id"], 
                           type=row["type"], **props)
    ...
    def add_node(self, node_id: str, label: str, properties: Dict = None) -> str:
        ...
        self.conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (node_id, label, node_data["properties"], node_data["created_at"], node_data["updated_at"]))
        return node_id
    
    def add_relationship(self, source: str, target: str, rel_type: str, properties: Dict = None) -> str:
...
ONTOLOGY = {
    "entities": {
        "Incident": {
            "properties": ["id", "ts_start", "ts_updated", "ts_cleared", "itype", "description", 
                          "location", "lat", "lon", "status", "confidence", "severity"],
            "description": "Clustered real-world event synthesized from calls"
        },
        "Call": {
            "properties": ["id", "ts", "tgid", "tag", "transcript", "duration", "lat", "lon", "location"],
        ...
        "Pattern": {
            "properties": ["id", "name", "description", "itype", "tgids", "confidence", "first_seen"],
    ...
    "relationships": {
        "PART_OF": {"from": "Call", "to": "Incident", "description": "Call contributes to incident"},
        "MENTIONS": {"from": "Call", "to": "Entity", "description": "Transcript mentions entity"},
        "INVOLVED": {"from": "Incident", "to": "Agency", "description": "Agency involved in incident"},
        ...
        "RELATED_TO": {"from": "Incident", "to": "Pattern", "description": "Incident exhibits pattern"},
        "LINKED_TO": {"from": "Incident", "to": "Incident", "description": "Related incidents (same event, escalation)"},
        "LOCATED_IN": {"from": ["Incident", "Entity"], "to": "Location", "description": "Geospatial link"}
    },
    "core_properties": {
