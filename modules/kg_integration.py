...
from kg_ontology import BattleBuddyKG
    ...
    try:
        ...
        with _kg_lock:
            ...
            if tgid is not None:
                ...
                kg.add_relationship(node_id, tg_node_id, "FROM",
                                    {"source": "live_pipeline"})
            ...
            if category and category != "Unknown":
                ...
                kg.add_relationship(node_id, agency_node_id, "BY",
                                    {"source": "live_pipeline"})

            kg.save()

        logger.info("kg_write_call: wrote call %s (tg:%s %s)",
                     call_id, tgid, call_data.get("tag", ""))
        return True

    except Exception as e:
        logger.error("kg_write_call: FAILED for call %s: %s",
                     call_data.get("id"), e, exc_info=True)
        return False
        ...
        with _kg_lock:
            ...
            for agency_name in incident_data.get("agencies", []):
                ...
                if not _node_exists(agency_node_id):
                    ...
                    kg.add_node(agency_node_id, label="Agency",
                                properties=agency_props)
                kg.add_relationship(node_id, agency_node_id, "INVOLVED",
                                    {"source": "live_pipeline"})

            kg.save()

        logger.info("kg_write_incident: wrote incident %s (%s) @ %s",
                     inc_id, itype, incident_data.get("location", "unknown"))
        return True

    except Exception as e:
        logger.error("kg_write_incident: FAILED for incident %s: %s",
                     incident_data.get("id"), e, exc_info=True)
        return False
...
def kg_write_transcript(call_id: int, transcript_text: str) -> bool:
    ...
    try:
        ...
        with _kg_lock:
            ...
            if existing is None:
                logger.debug("kg_write_transcript: call node %s not found yet; "
                             "will be picked up on next kg_write_call()", call_id)
        ...
        logger.info("kg_write_transcript: updated transcript on call %s "
                    "(%d chars)", call_id, len(transcript_text or ""))
        return True

    except Exception as e:
        logger.error("kg_write_transcript: FAILED for call %s: %s",
    ...
    try:
        ...
        with _kg_lock:
            ...
            rel_id = kg.add_relationship(
                call_node, inc_node, "PART_OF",
                {"source": "live_pipeline", "created_at": time.time()}
            )
            kg.save()

        logger.info("kg_link_call_to_incident: call %s → PART_OF → incident %s",
                     call_id, incident_id)
        return True
